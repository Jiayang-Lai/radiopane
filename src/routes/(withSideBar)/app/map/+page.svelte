<script lang="ts">
	...
	import { fetchGeoStations, type AppStation } from '$lib/radio-browser';
  ...
  import Button from '$lib/components/ui/button/button.svelte';
	...
	let isLoadingStations = $state(true);
	...
	onMount(() => {
		...
		async function setupMap() {
			...
			const [L, stations] = await Promise.all([import('leaflet'), fetchGeoStations(fetch)]);
		...
		void setupMap().catch(() => {
			...
			loadError = 'Unable to load radio stations for the map right now.';
			selectedCluster = null;
			showAllClusterStations = false;
			isLoadingStations = false;
		});
...
<section class="flex min-h-0 flex-1 flex-col gap-2 sm:gap-3 flex-start">
	...
	<div class="grid min-h-0 flex-1 gap-2 sm:gap-3 xl:grid-cols-[minmax(0,1fr)_22rem]">
